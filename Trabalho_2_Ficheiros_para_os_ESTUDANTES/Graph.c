...
Graph* GraphCreateTranspose(const Graph* g) {
    ...
    for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(vertices), i++) {
        ...
        List* edges = vPointer->edgesList;
        ListMoveToHead(edges);
        for (unsigned int j = 0; j < ListGetSize(edges); ListMoveToNext(edges), j++) {
            struct _Edge* ePointer = ListGetCurrentItem(edges);
            if (g->isWeighted) {
                GraphAddWeightedEdge(transpose, ePointer->adjVertex, vPointer->id, ePointer->weight);
            } else {
                GraphAddEdge(transpose, ePointer->adjVertex, vPointer->id);
            }
        }
    }
